...
const ChatBox = () => {
  ...
  const onSubmit = async (e) => {
    ...
    // user ka message push karo
    const newMessage = { role: 'user', content: prompt, timestamp: Date.now(), isImage: false };
    ...
    try {
      ...
      if (data.success) {
        // ✅ reply ko isImage ke sath set karo
        const replyMsg = {
          ...data.reply,
          isImage: mode === "image"
        };
        ...
        // ✅ chats context update
        setChats(prevChats =>
          prevChats.map(chat =>
            chat._id === selectedChat._id
              ? { ...chat, messages: [...chat.messages, newMessage, replyMsg], updatedAt: Date.now() }
              : chat
          )
        );
      } else {
